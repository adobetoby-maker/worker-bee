...
function JayCockpit() {
  ...
  const send = () => {
    ...
    setInput("");
    setThoughts((t) => [...t, { ts: at, kind: "think" as const, text: `parsing prompt (${text.length} chars)` }].slice(-200));
    window.setTimeout(() => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-background text-foreground font-mono overflow-hidden">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[minmax(0,1.1fr)_320px_minmax(0,1.4fr)]">
        <section className="flex flex-col min-h-0 border-b lg:border-b-0 lg:border-r" style={{ borderColor: "var(--border)" }}>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2 text-[12px] leading-relaxed space-y-2">
            {chat.map((m, i) => (
              <div key={i} className="animate-fade-in">
                ...
                <div className={m.role === "you" ? "text-foreground" : "text-success"}>{m.text}</div>
              </div>
            ))}
            <div ref={chatEndRef} />
          </div>
          <div className="shrink-0 border-t p-2 flex gap-2" style={{ borderColor: "var(--border)" }}>
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Talk to the bee…" className="flex-1 px-3 py-2 rounded border bg-background text-[12px]" style={{ borderColor: "var(--border)" }} />
