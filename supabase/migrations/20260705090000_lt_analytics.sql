-- COPY of supabase/lt-analytics.sql (original kept in place), timestamped so
-- it lives in migration history. This schema was previously applied manually
-- via the Supabase SQL editor — this file is written to be idempotent so a
-- `supabase db push` against a database that already has these objects is a
-- no-op:
--   * create table / create index: original already used IF NOT EXISTS — unchanged
--   * alter table ... enable row level security: natively idempotent — unchanged
--   * create policy: has no IF NOT EXISTS in Postgres, so each policy is
--     wrapped in a DO block that swallows duplicate_object. Statement bodies
--     are otherwise byte-identical to the original.
--
